...
def "switch project" [] {
  ...
  let selected_project_name = ($selected_project_path | path split | last)
  let sessions = (tmux list-sessions | lines | each {|l| $l | split row : | first })

  # Create the session if it doesn't exist
  if ($sessions | find $selected_project_name | is-empty) {
    tmux new-session -c $selected_project_path -d -s $selected_project_name 
  } 
    
  try {
    tmux switch-client -t $selected_project_name
  } catch {
    tmux new -c $selected_project_path -A -s $selected_project_name
  }
}
...
# Assorted az cli things
def "az list sub" [] {
  az account list --all | from json | select name id state
}

def "az select sub" [name?: string] {
  let subs = (az account list --all | from json)
  let selected_name = (
    if ($name == null) { $subs | get name | to text | fzf } else { $name }
    | str kebab-case
  )
  let id = (
    $subs 
    | filter {|s| ($s.name | str kebab-case) == $selected_name } 
    | get id
  )
  az account set -s $id 
  az account show
...
def flink-k8s [] {
  let job_manager = (kubectl --kubeconfig $env.KUBECONFIG get pods --namespace flink 
    | from ssv 
    | where { $in.NAME | str starts-with flink-session-cluster- } 
    | first 
    | get NAME)

  $"Found jobmanager: $job_manager"

  kubectl --kubeconfig $env.KUBECONFIG exec -it $job_manager -n flink -- bash
}
...
def "az keyvault search" [keyvault?: string] {
  let keyvault = (if ($keyvault == null) { az keyvault list | from json | get name | to text | fzf } else { $keyvault })
  let secret_name = (az keyvault secret list --vault-name $keyvault | from json | get name | to text | fzf)
  if ($secret_name == null) { return }
